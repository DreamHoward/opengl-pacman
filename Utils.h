...
namespace Utils
{
	...
	int manhattanDistance(glm::vec2 a, glm::vec2 b)
	{
		return fabs(a.x - b.x) + fabs(a.y - b.y);
	}

	glm::vec2 aStar(const glm::vec2& start, const glm::vec2& end, const Map& map)
	{
		// Boundary check
		...
		for (int i = 0; i < map.GetHeight(); i++)
		{
			...
			for (int j = 0; j < map.GetWidth(); j++)
				h[i].push_back(manhattanDistance(glm::vec2(j, i), end));
		}
		g[start.y][start.x] = 0;
