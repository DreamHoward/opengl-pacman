...
namespace Utils
{
	...
	glm::vec3 MgridPosToWorldPos(glm::vec2 gridPos)
	{
		return glm::vec3(gridPos.x, 0.0f, gridPos.y) * 100.0f + 50.0f;
	}

	glm::vec2 MworldPosToGridPos(glm::vec3 worldPos)
	{
		return glm::vec2(floor(worldPos.x / 100.0f),
			floor(worldPos.z / 100.0f));
	}

	int MmanhattanDistance(glm::vec2 a, glm::vec2 b)
	{
		return fabs(a.x - b.x) + fabs(a.y - b.y);
	}

	glm::vec2 MaStar(const glm::vec2& start, const glm::vec2& end, const Map& map)
	{
		// Boundary check
		...
		for (int i = 0; i < map.GetHeight(); i++)
		{
			...
			for (int j = 0; j < map.GetWidth(); j++)
				h[i].push_back(MmanhattanDistance(glm::vec2(j, i), end));
		}
		g[start.y][start.x] = 0;
